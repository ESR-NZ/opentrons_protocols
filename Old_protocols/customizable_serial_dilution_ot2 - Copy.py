...
def run(protocol_context):
    ...
    ## Standards to every second well col-1 in plate 
    pipette_p200.distribute(
        diluent_volume-standard_vol, trough.wells()[0], [well for well in plate.columns()[0]][1::2])
    pipette_p20.distribute(
        standard_vol, standard_tube, [well for well in plate.columns()[0]][1::2])


    ## dilutant to empty col 1 wells in plate 
    pipette_p200.distribute(
        diluent_volume, trough.wells()[0], [well for well in plate.columns()[0]][::2])
    ...
    for row in plate.rows()[1::2]:
        
        if tip_use_strategy == 'never':
            pipette_p200.pick_up_tip()


        for s, d in zip(row[:num_of_dilutions*2:2], row[1:1+num_of_dilutions*2:2]):
